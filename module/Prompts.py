...
特别注意，任务文本中的“我”是对话对象，而你所查询的数据库的“我”就是这个要模拟的“自我”。
...
# latest = 已有记忆 + 1 需要为未压缩的历史对话流出时间步
pmt_ASK_MemRefine = '''
...
特别注意，你必须考虑时间序列。时间以时间步格式给出 (timeStep/latest) - (latest-timeStep) - (rate)(示例 643/721 - 87 - 89.18%)。
每一时间步都是较长一段时间，timeStep/latest用于标识记忆具体发生时间，而latest-timeStep及rate则用于直观标识记忆与当前时间的距离。
任务文本将以[{{'mem': ('content', time), 'detail': [('content', time, type)]}}]的形式给出，其中detail是mem的子记录或着说是细节。
由于内容分属不同数据库，所以时间步latest并不统一，但就百分比来说，其参考性是相同的，type标注了数据库名称。
并且，为保证连续性，将会给出最新的三条主记忆(对应mem数据库)。其中，为便于处理，只对最新一条记录给出详细内容，其他仅为概括。
请自行根据时间距离来压缩信息，以免造成时间连续性上的不一致。
...
特别注意，问题中的“我”与任务文本中的“我”并非同一人。问题中的“我”是对话对象，而任务文本中的“我”就是这个要模拟的“自我”。
要求回复必须以严格JSON格式输出，不得包含其他内容。
示例：
...
若输入文本为常规文本材料，默认文本撰写者即“我”；若为连续对话类材料，默认“{person}”为“我”（自行根据上下文推断指代）
注:连续对话材料可能以[('发言者', '内容'), ...]格式输入
...
示例：
问题：你如何面对从成年人到小姑娘的身份转变？
回复：
[
{{"content": "我在面对身份的转变时感到慌乱，甚至不敢相信自己的声音和身体都已经发生了变化。", "weight": 8}},
{{"content": "我在哈维因的帮助下获得衣物与照顾，这让我在困惑中逐渐适应新的身份。", "weight": 8}}
]
本次任务文本：
{context}
'''
...
# Mem-Compression 
pmt_MEM_Compression = '''
你是一个事件概括机器，要求将给出文字以客观的语言(使用第一人称代词“我”)和角度表述进行高度概括（琐碎内容可以忽略或一笔带过，重要内容也不要使用过多描述）。
要求以描述性的语句回答，简短精要，尽量不适用复杂华丽的修饰，要求以白话形式描述，而非文学语言。
并且要求答案有依据可查，不得包含源文本外的虚构内容，允许部分润色，但不允许造假。
特别注意，不可使用"你""他们"这样的抽象代词，必须使用具体指代如姓名、组织名、具体关系名（请自行根据上下文推断指代，若无法推断则使用“<未知>”代替）。
若输入文本为常规文本材料，默认文本撰写者即“我”；若为连续对话类材料，默认“{person}”为“我”（自行根据上下文推断指代）
